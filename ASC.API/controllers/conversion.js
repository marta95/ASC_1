...
const modularRecordExcess = async function(args){
    try {
        var Excess = [];
        Excess = await number2Excess3(args);
        const arrayConverted = [];
        if (Excess[0] == "0.") {
            arrayConverted.push(Excess);
        } else {
            arrayConverted.push(Excess);
        }
        return arrayConverted.toString().split(",");
...
const oppositeRecordExcess = async function(args){
    try {
        var array = [];
        var arrayConverted = [];
        arrayConverted = await number2Excess3(args);
    } catch (err) { }
...
const objectiveRecordExcess = async function (args){
    try {
        var arrayOpposite = [];
        arrayOpposite = await oppositeRecordExcess(args);
        var array = [];
    } catch (err) { }
    if (arrayOpposite[0] == "0.") {
        array.push(arrayOpposite);
...
module.exports = {
    ...
    modularRecordExcess: modularRecordExcess,
    oppositeRecordExcess: oppositeRecordExcess,
    objectiveRecordExcess: objectiveRecordExcess,
}